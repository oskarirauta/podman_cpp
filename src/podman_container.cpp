...
const bool Podman::podman_t::update_containers(void) {
	...
	if ( logger::log_level >= logger::debug.id())
		logger::vverbose << "updating containers" << std::endl;
	...
	if ( !socket.execute(query, response)) {
		...
		logger::debug << "update containers failed, containers array state set to incomplete" << std::endl;
	...
	if ( hashValue == this -> hash.containers ) {
		...
		Podman::mutex.podman.unlock();
		if ( logger::log_level >= logger::debug.id())
			logger::debug << "container state hash remained the same, success without updating pods and containers" << std::endl;
		return true;
	}
	...
	for ( int i = 0; i < array_size; i++ ) {
		...
		if ( !this -> parse_cntr(jvalue, new_cntr)) {
			logger::debug << "container parser failure - skipped member #" << i << std::endl;
			continue;
		}
