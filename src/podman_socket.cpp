...
static size_t receiveCallback(void *contents, size_t size, size_t nmemb, void *response) {
	...
	if ( logger::log_level >= logger::debug.id() && ptr -> chunks_allowed != 0 )
		logger::debug << "received chunk #" << ptr -> chunks << std::endl;
...
bool Podman::Socket::execute(Podman::Query query, Podman::Query::Response &response) {
	...
	if ( !curl ) {
		...
		logger::vverbose << "error with socket when querying " << common::trim_leading(query.path()) << ": socket not available" << std::endl;
		curl_easy_cleanup(curl);
		return false;
	}

	if ( logger::log_level >= logger::debug.id())
		logger::debug << "socket: " << this -> path << " query: " << query.url() << std::endl;
	...
	if ( res != CURLE_OK ) {
		...
		logger::vverbose << "socket failed with error while querying for " << common::trim_leading(query.path()) << ": " << this -> error.description() << std::endl;
		curl_easy_cleanup(curl);
		return false;
	}
	...
	if ( response.body.length() != 0 ) {
		...
		if ( query.parseJson && !response.parseJson()) {
			...
			logger::vverbose << "error: " << this -> error.description() << std::endl;
			return false;
		}

		return true;
	} else if ( logger::log_level >= logger::debug.id())
		logger::vverbose << "received response from socket had size of 0, received nothing for query " << common::trim_leading(query.path()) << std::endl;
