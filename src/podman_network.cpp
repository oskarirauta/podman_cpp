...
const bool Podman::podman_t::update_networks(void) {
	...
	if ( logger::log_level >= logger::debug.id())
		logger::vverbose << "updating networks" << std::endl;
	...
	for ( int i = 0; i < array_size; i++ ) {
		...
		if ( name.empty() || id.empty()) {
			...
			if ( name.empty() && id.empty())
				logger::debug << "error while parsing networks, name and id objects are missing, or invalid format" << std::endl;
			else if ( name.empty())
				logger::debug << "error while parsing networks, name object is missing or invalid format" << std::endl;
			else if ( id.empty())
				logger::debug << "error while parsing networks, id object is missing or invalid format" << std::endl;
		...
		if ( struct json_object *jsubnets = json_object_object_get(jnet, "subnets"); json_object_is_type(jsubnets, json_type_array)) {
			...
			for ( int i2 = 0; i2 < subnets_size; i2++ ) {
				...
				if ( !json_object_is_type(jsubnet, json_type_object)) {
					logger::debug << "error while parsing networks, invalid subnet" << std::endl;
					continue;
				}
				...
				if ( subnet.subnet.empty() && subnet.gateway.empty()) {
					logger::debug << "error, invalid subnet for network " << new_net.name << std::endl;
					continue;
				}
			...
			else logger::vverbose << "warning, no subnets found for network " << new_net.name << std::endl;

		} else logger::vverbose << "error while parsing network " << new_net.name << " subnets" << std::endl;
		...
		if ( struct json_object *driver = json_object_object_get(jnet, "driver"); json_object_is_type(driver, json_type_string))
			...
			logger::vverbose << "warning, unable to parse driver for network " << new_net.name << std::endl;
		}
		...
		if ( new_net.ipam_driver.empty()) {
			...
			logger::vverbose << "warning, unable to parse ipam driver for network " << new_net.name << std::endl;
		}
		...
		else logger::vverbose << "warning, unable to parse network interface for network " << new_net.name << std::endl;
	...
	if ( new_networks.empty()) {
		logger::vverbose << "error: no networks, this can't be right.." << std::endl;
		return false;
	}
