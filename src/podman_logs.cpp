...
bool Podman::Container::update_logs(Podman::Socket *socket) {
	...
	if ( !this -> isRunning ) {
		if ( logger::log_level >= logger::debug.id())
			logger::debug << "logs not updated for container " << this -> name << ", container is not running" << std::endl;
		return true;
	}
...
const bool Podman::podman_t::update_logs(void) {
	...
	if ( logger::log_level >= logger::debug.id())
		logger::debug << "updating logs" << std::endl;

	mutex.podman.lock();
	if ( this -> state.containers != Podman::Node::OK ) {
		logger::vverbose << "error: failed to update logs, container array not ready" << std::endl;
		mutex.podman.unlock();
		return false;
	}
	...
	for ( int p = 0; p < pods.size(); p++ ) {
		for ( int c = 0; c < pods[p].containers.size(); c++ ) {
			if ( !pods[p].containers[c].update_logs(&this -> socket)) {
				logger::vverbose << "error: failed to retrieve logs for " << pods[p].containers[c].name << std::endl;
				return false;
			}
		}
	}
